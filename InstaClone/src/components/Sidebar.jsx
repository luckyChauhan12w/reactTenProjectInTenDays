    ...
    <div className="fixed left-0 top-0 h-full w-64 bg-white border-r border-gray-200 z-40 hidden lg:block">
      ...
      <nav className="py-4">
        {navigationItems.map((item) => (
          ...
          >
            <div className={`${item.active ? 'text-black' : 'text-gray-700'}`}>
              {item.icon}
            </div>
            <span className={`text-base ${item.active ? 'text-black' : 'text-gray-700'}`}>
              {item.label}
            </span>
          </button>
        ))}
      ...
      <div className="absolute bottom-6 left-0 right-0 px-6">
        <button className="w-full flex items-center gap-4 px-0 py-3 text-left hover:bg-gray-50 transition-colors rounded-lg">
